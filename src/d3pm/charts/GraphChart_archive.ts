...
export interface GraphChartOptions extends BaseChartOptions {
  layout?: 'force' | 'reverse' | 'dot' | 'neato' | 'circo' | 'fdp' | 'sfdp' | 'twopi';
  nodeSize?: number;
  edgeWidth?: number;
  arrowSize?: number;
}
...
export class GraphChart extends BaseChart<{nodes: GraphNode[], edges: GraphEdge[]}, GraphChartOptions> {
  ...
  private sharedTargetToSources: Map<string, string[]> = new Map(); // Store shared target information
  ...
  private computeLayout(): void {
    ...
    // Check if this is a GraphViz layout algorithm
    const graphvizLayouts = ['dot', 'neato', 'circo', 'fdp', 'sfdp', 'twopi'];
    
    if (graphvizLayouts.includes(layout)) {
      this.computeGraphVizLayout(layout);
  ...
  private computeGraphVizLayout(algorithm: string): void {
    try {
      // Use GraphViz to calculate optimal positions synchronously
      const result = this.executeGraphVizSync(algorithm);
      
      // Update nodes with GraphViz-calculated positions
      this.processedNodes = result.nodes;
      
      // Update chart dimensions if needed
      if (!this.explicitDimensions.width || !this.explicitDimensions.height) {
        const padding = 40; // Add some padding around the graph
        if (!this.explicitDimensions.width) {
          this.options.width = result.bbox.width + padding * 2;
        }
        if (!this.explicitDimensions.height) {
          this.options.height = result.bbox.height + padding * 2;
        }
      ...
      console.warn(`GraphViz layout failed: ${error.message}. Falling back to force layout.`);
      this.computeForceLayout();
    }
  }
  
  private executeGraphVizSync(algorithm: string): {nodes: GraphNode[], edges: GraphEdge[], bbox: {width: number, height: number}} {
    // Convert d3pm format to DOT format
    const dotGraph = this.convertToDot(this.processedNodes, this.processedEdges, algorithm);
    
    // Execute GraphViz synchronously using Deno.Command with stdout capture
    const tempDir = '/tmp';
    const dotFile = `${tempDir}/graph_${Date.now()}.dot`;
    
    // Write DOT content to file
    Deno.writeTextFileSync(dotFile, dotGraph);
    
    try {
      // Execute GraphViz dot command with JSON output
      const command = new Deno.Command('dot', {
        args: ['-Tjson', dotFile],
        stdout: 'piped',
        stderr: 'piped'
      });
      ...
      if (code !== 0) {
        ...
        throw new Error(`GraphViz execution failed: ${errorText}`);
      }
      
      const jsonOutput = new TextDecoder().decode(stdout);
      const graphvizOutput = JSON.parse(jsonOutput);
      
      return this.convertFromGraphViz(graphvizOutput, this.processedNodes, this.processedEdges);
      
    } finally {
      // Clean up temporary file
      try {
        Deno.removeSync(dotFile);
      } catch {
  ...
  private convertToDot(nodes: GraphNode[], edges: GraphEdge[], algorithm: string): string {
    const nodeAttr = algorithm === 'dot' ? '[shape=box]' : '[shape=circle]';
    
    const dotNodes = nodes.map(node => {
      const shape = node.shape === 'rect' ? 'box' : 'circle';
      const label = node.text.replace(/"/g, '\\"');
      return `  "${node.id}" [label="${label}", shape=${shape}]`;
    }).join('\n');
    
    const dotEdges = edges.map(edge => {
  ...
  private convertFromGraphViz(
    graphvizOutput: GraphVizOutput, 
    originalNodes: GraphNode[], 
    originalEdges: GraphEdge[]
  ): {nodes: GraphNode[], edges: GraphEdge[], bbox: {width: number, height: number}} {
    // Parse bounding box
    const [x1, y1, x2, y2] = graphvizOutput.bb.split(',').map(Number);
    const bbox = { width: x2 - x1, height: y2 - y1 };
    
    // Create mapping from GraphViz nodes to original nodes
    const nodeMap = new Map<string, GraphVizNode>();
    graphvizOutput.objects.forEach(gvNode => {
      nodeMap.set(gvNode.name, gvNode);
    });
    
    // Update node positions
    const positionedNodes = originalNodes.map(node => {
      const gvNode = nodeMap.get(node.id);
      if (gvNode && gvNode.pos) {
        const [x, y] = gvNode.pos.split(',').map(Number);
        // GraphViz uses bottom-left origin, we need top-left
        const flippedY = (y2 - y1) - (y - y1);
        return {
          ...node,
          x: x - x1, // Normalize to start at 0
          y: flippedY
        };
      }
      return node;
    ...
    return {
      ...
      edges: originalEdges, // Keep original edges for now
      bbox
    };
  }
  ...
  private calculateGlobalYPositions(
    ...
    // Phase 4: FINAL REPOSITIONING - Now that all nodes are positioned, reposition shared inputs and targets
    if (this.debug) console.log('\n🎯 Final phase: Repositioning shared inputs and targets...');
    this.repositionSharedInputsOptimally(inEdges, outEdges, processedNodes);
  ...
  private positionLeafInputsOptimally(
    ...
    // NEW: Handle shared inputs first, then fall back to parallel processing logic
    
    if (this.debug) console.log(`🔧 Positioning ${leafInputs.length} leaf inputs optimally...`);
    
    // Step 1: Detect shared inputs (inputs that connect to multiple target nodes)
    const sharedInputToTargets = this.detectSharedInputs(leafInputs, inEdges, outEdges);
    
    // Step 2: Detect shared targets (nodes that receive from multiple sources)
    const sharedTargetToSources = this.detectSharedTargets(inEdges, outEdges);
    
    // Step 3: Separate shared inputs from node-specific inputs
    const sharedInputs = Array.from(sharedInputToTargets.keys());
    const nonSharedInputs = leafInputs.filter(inputId => !sharedInputs.includes(inputId));
    
    if (this.debug) {
      console.log(`🔗 Found ${sharedInputs.length} shared inputs: [${sharedInputs.join(', ')}]`);
      console.log(`🎯 Found ${sharedTargetToSources.size} shared targets: [${Array.from(sharedTargetToSources.keys()).join(', ')}]`);
      console.log(`🔧 Found ${nonSharedInputs.length} node-specific inputs: [${nonSharedInputs.join(', ')}]`);
    }
    
    // Step 4: Store shared information for later repositioning
    this.sharedTargetToSources = sharedTargetToSources;
    
    // Step 5: Handle ALL leaf inputs (both shared and non-shared) using the same logic
    // We'll let the column-based positioning handle them initially, then reposition shared ones later
    const allLeafInputs = leafInputs; // Process all inputs the same way initially
    
    if (allLeafInputs.length > 0) {
      this.positionNonSharedInputs(allLeafInputs, inEdges, outEdges, margin, minNodeSpacing, processedNodes);
    }
    
    // Step 6: Mark that we have shared inputs/targets for later repositioning
    // The actual repositioning will happen after all nodes are positioned by the column algorithm
    if (this.debug && (sharedInputs.length > 0 || sharedTargetToSources.size > 0)) {
      console.log(`📝 Marked for later repositioning: ${sharedInputs.length} shared inputs, ${sharedTargetToSources.size} shared targets`);
    }
  }

  private positionSharedInputsInitially(
    sharedInputs: string[],
    sharedInputToTargets: Map<string, string[]>,
    margin: number,
    minNodeSpacing: number,
    processedNodes: Set<string>
  ): void {
    // Initial positioning for shared inputs - will be refined after target nodes are positioned
    
    if (this.debug) console.log(`🔗 Initial positioning of ${sharedInputs.length} shared inputs...`);
    
    let currentY = margin + 20; // Start with some offset
    
    // Sort shared inputs deterministically for consistent positioning
    const sortedSharedInputs = [...sharedInputs].sort();
    
    sortedSharedInputs.forEach((inputId, index) => {
      const targetIds = sharedInputToTargets.get(inputId) || [];
      
      // Position with spacing that accounts for multiple targets
      const yPosition = currentY + (index * minNodeSpacing * 2); // Extra spacing for shared inputs
      
      this.nodeYPositions.set(inputId, yPosition);
      processedNodes.add(inputId);
      
      if (this.debug) console.log(`🔗 Initially positioned shared input ${inputId} at Y=${yPosition} (connects to [${targetIds.join(', ')}])`);
    });
  }

  private positionNonSharedInputs(
    nonSharedInputs: string[],
    inEdges: Map<string, string[]>,
    outEdges: Map<string, string[]>,
    margin: number,
    minNodeSpacing: number,
    ...
    // Position operation-specific inputs using collision-aware logic
    
    if (this.debug) console.log(`🔧 Positioning ${nonSharedInputs.length} operation-specific inputs with collision avoidance...`);
    
    // Get all operations for spacing calculation
    const operations = this.processedNodes.filter(node => node.shape === 'circle');
    const optimalSpacing = this.calculateOptimalOperationSpacing(operations, inEdges, minNodeSpacing);
    
    // Group non-shared inputs by their target operations
    const inputsByOperation = new Map<string, string[]>();
    
    nonSharedInputs.forEach(inputId => {
      // Find which operation this input connects to
      const connectedOps = outEdges.get(inputId)?.filter(targetId => 
        operations.some(op => op.id === targetId)
      ) || [];
      
      if (connectedOps.length > 0) {
        const opId = connectedOps[0]; // Take first operation
        if (!inputsByOperation.has(opId)) {
          inputsByOperation.set(opId, []);
        }
        inputsByOperation.get(opId)!.push(inputId);
      }
    });
    
    if (this.debug) {
      console.log(`🔧 Grouped inputs by operation:`);
      inputsByOperation.forEach((inputs, opId) => {
        console.log(`   ${opId}: [${inputs.join(', ')}]`);
      });
    }
    
    // Position inputs for each operation group, avoiding conflicts
    // Calculate available height for positioning
    const { innerHeight } = this.dimensions;
    const availableHeight = innerHeight - (2 * margin);
    const numOperationGroups = inputsByOperation.size;
    
    // Adjust spacing to fit within available height
    const maxSpacing = numOperationGroups > 1 ? availableHeight / (numOperationGroups - 1) : 0;
    const actualSpacing = Math.min(optimalSpacing, maxSpacing);
    
    if (this.debug) console.log(`📐 Spacing adjustment: optimal=${optimalSpacing}px, max=${maxSpacing.toFixed(1)}px, actual=${actualSpacing.toFixed(1)}px`);
    
    let currentOperationY = margin + 30; // Start with reasonable margin
    
    inputsByOperation.forEach((inputs, operationId) => {
      if (this.debug) console.log(`🔧 Positioning inputs for operation ${operationId} at Y=${currentOperationY}`);
      
      // Position inputs around the operation center
      this.positionInputsAroundOperation(inputs, operationId, currentOperationY, minNodeSpacing, processedNodes);
      
      // Move to next operation position, ensuring we don't exceed bounds
      currentOperationY += actualSpacing;
      
      // Ensure we don't go beyond available space
      if (currentOperationY > innerHeight - margin - 30) {
        currentOperationY = innerHeight - margin - 30;
      }
    });
  }

  private positionInputsAroundOperation(
    inputs: string[],
    operationId: string,
    operationCenterY: number,
    minNodeSpacing: number,
    processedNodes: Set<string>
  ): void {
    // Position inputs around an operation center, avoiding conflicts
    ...
    if (inputs.length === 1) {
      // Single input: position slightly offset from operation center
      const inputY = this.findSafeYPosition(operationCenterY - minNodeSpacing / 2, minNodeSpacing);
      this.nodeYPositions.set(sortedInputs[0], inputY);
      processedNodes.add(sortedInputs[0]);
      
      if (this.debug) console.log(`   Single input ${sortedInputs[0]} positioned at Y=${inputY}`);
    } else if (inputs.length === 2) {
      // Pair of inputs: position symmetrically around operation center
      const spacing = minNodeSpacing * 0.8; // Slightly tighter spacing
      const input1Y = this.findSafeYPosition(operationCenterY - spacing / 2, minNodeSpacing);
      const input2Y = this.findSafeYPosition(operationCenterY + spacing / 2, minNodeSpacing);
      
      this.nodeYPositions.set(sortedInputs[0], input1Y);
      this.nodeYPositions.set(sortedInputs[1], input2Y);
      processedNodes.add(sortedInputs[0]);
      processedNodes.add(sortedInputs[1]);
      
      if (this.debug) console.log(`   Input pair: ${sortedInputs[0]} at Y=${input1Y}, ${sortedInputs[1]} at Y=${input2Y}`);
    } else {
      // Multiple inputs: distribute around center
      const totalSpacing = (inputs.length - 1) * minNodeSpacing * 0.7;
      const startY = operationCenterY - totalSpacing / 2;
      
      sortedInputs.forEach((inputId, index) => {
        const inputY = this.findSafeYPosition(startY + index * minNodeSpacing * 0.7, minNodeSpacing);
        this.nodeYPositions.set(inputId, inputY);
        processedNodes.add(inputId);
        
        if (this.debug) console.log(`   Input ${inputId} (${index + 1}/${inputs.length}) positioned at Y=${inputY}`);
      });
    }
  }

  private findSafeYPosition(preferredY: number, minNodeSpacing: number): number {
    // Find a safe Y position that doesn't conflict with existing nodes and stays within bounds
    ...
    // Clamp preferred position to bounds
    let clampedPreferredY = Math.max(minY, Math.min(maxY, preferredY));
    
    const occupiedPositions = Array.from(this.nodeYPositions.values()).sort((a, b) => a - b);
    const safeDistance = minNodeSpacing * 0.8; // Minimum safe distance
    
    // Check if clamped preferred position is safe
    const isSafe = occupiedPositions.every(pos => Math.abs(pos - clampedPreferredY) >= safeDistance);
    ...
    // Try positions above and below, respecting bounds
    for (let offset = safeDistance; offset <= minNodeSpacing * 3; offset += safeDistance / 2) {
      const candidates = [clampedPreferredY + offset, clampedPreferredY - offset];
      
      for (const candidateY of candidates) {
        // Skip candidates outside bounds
        if (candidateY < minY || candidateY > maxY) continue;
        
        const candidateIsSafe = occupiedPositions.every(pos => Math.abs(pos - candidateY) >= safeDistance);
        
        if (candidateIsSafe) {
          const distance = Math.abs(candidateY - clampedPreferredY);
    ...
    if (this.debug && bestY !== preferredY) {
      console.log(`   Adjusted position: ${preferredY.toFixed(1)} → ${bestY.toFixed(1)} (offset: ${(bestY - preferredY).toFixed(1)})`);
    }
  ...
  private detectSharedInputs(
    leafInputs: string[],
    inEdges: Map<string, string[]>,
    outEdges: Map<string, string[]>
  ): Map<string, string[]> {
    // Analyze which leaf inputs are shared across multiple target nodes
    // Returns a map of sharedInput -> target nodes that use it
    
    const sharedInputToTargets = new Map<string, string[]>();
    // Consider ALL nodes as potential targets, not just operations (circles)
    const allNodes = this.processedNodes;
    
    leafInputs.forEach(inputId => {
      const connectedTargets: string[] = [];
      
      // Find all nodes that use this input
      allNodes.forEach(node => {
        const nodeInputs = inEdges.get(node.id) || [];
        if (nodeInputs.includes(inputId)) {
          connectedTargets.push(node.id);
        }
      });
      
      // Only consider inputs that are shared (connect to multiple targets)
      if (connectedTargets.length > 1) {
        sharedInputToTargets.set(inputId, connectedTargets);
        if (this.debug) console.log(`🔗 Shared input detected: ${inputId} → [${connectedTargets.join(', ')}]`);
      }
    });
    
    return sharedInputToTargets;
  }

  private detectSharedTargets(
    inEdges: Map<string, string[]>,
    outEdges: Map<string, string[]>
  ): Map<string, string[]> {
    // Analyze which nodes receive inputs from multiple source nodes
    // Returns a map of sharedTarget -> source nodes that feed into it
    
    const sharedTargetToSources = new Map<string, string[]>();
    
    this.processedNodes.forEach(node => {
      const sources = inEdges.get(node.id) || [];
      
      // Only consider nodes that receive from multiple sources
      if (sources.length > 1) {
        sharedTargetToSources.set(node.id, sources);
        if (this.debug) console.log(`🎯 Shared target detected: ${node.id} ← [${sources.join(', ')}]`);
      }
    });
    
    if (this.debug) console.log(`🎯 detectSharedTargets found ${sharedTargetToSources.size} shared targets`);
    return sharedTargetToSources;
  }

  private calculateOperationCenterY(
    operationIds: string[],
    processedNodes: Set<string>
  ): number | undefined {
    // Calculate the center Y position of a group of operations
    // Returns undefined if operations aren't positioned yet
    
    const positionedOperations = operationIds.filter(opId => this.nodeYPositions.has(opId));
    
    if (positionedOperations.length === 0) {
      return undefined; // Operations not positioned yet
    }
    
    const totalY = positionedOperations.reduce((sum, opId) => {
      return sum + this.nodeYPositions.get(opId)!;
    }, 0);
    
    const centerY = totalY / positionedOperations.length;
    if (this.debug) console.log(`📐 Operation center Y: [${positionedOperations.join(', ')}] → ${centerY.toFixed(1)}`);
    
    return centerY;
  }

  private positionSharedInputs(
    sharedInputToOperations: Map<string, string[]>,
    inEdges: Map<string, string[]>,
    outEdges: Map<string, string[]>,
    margin: number,
    minNodeSpacing: number,
    ...
    // Position shared inputs at the center of their connected operations
    
    if (sharedInputToOperations.size === 0) {
      if (this.debug) console.log(`🔗 No shared inputs detected, skipping shared positioning`);
      return;
    }
    
    if (this.debug) console.log(`🔗 Positioning ${sharedInputToOperations.size} shared inputs...`);
    
    // First, we need to position the operations to know where to center the shared inputs
    // For now, we'll defer shared input positioning until operations are positioned
    
    sharedInputToOperations.forEach((operationIds, inputId) => {
      // Try to calculate center of operations
      const centerY = this.calculateOperationCenterY(operationIds, processedNodes);
      
      if (centerY !== undefined) {
        // Position shared input at center of its operations
        this.nodeYPositions.set(inputId, centerY);
        processedNodes.add(inputId);
        if (this.debug) console.log(`🔗 Positioned shared input ${inputId} at center Y=${centerY.toFixed(1)} of operations [${operationIds.join(', ')}]`);
      } else {
        // Operations not positioned yet - we'll need to handle this in a second pass
        if (this.debug) console.log(`🔗 Deferring positioning of shared input ${inputId} - operations not positioned yet`);
      }
    });
  }
  ...
  private processIncrementally(
    ...
    // Second pass: Reposition shared inputs optimally now that operations are positioned
    this.repositionSharedInputsOptimally(inEdges, outEdges, processedNodes);
  }

  private calculateOptimalOperationSpacing(
    operations: GraphNode[],
    inEdges: Map<string, string[]>,
    minNodeSpacing: number
  ): number {
    // Calculate optimal spacing between operations to prevent input overlap
    
    // Find maximum number of inputs per operation
    let maxInputsPerOperation = 0;
    operations.forEach(op => {
      const inputs = inEdges.get(op.id) || [];
      maxInputsPerOperation = Math.max(maxInputsPerOperation, inputs.length);
    });
    
    // Base spacing requirement: each operation needs space for its inputs
    const baseSpacing = minNodeSpacing * 2; // Standard minimum
    
    // Additional spacing for operations with many inputs
    const inputSpacingMultiplier = Math.max(1, maxInputsPerOperation - 1);
    const additionalSpacing = minNodeSpacing * inputSpacingMultiplier * 0.5;
    
    const optimalSpacing = baseSpacing + additionalSpacing;
    
    if (this.debug) console.log(`📐 Optimal operation spacing: ${optimalSpacing}px (base: ${baseSpacing}, additional: ${additionalSpacing}, maxInputs: ${maxInputsPerOperation})`);
    
    return Math.max(optimalSpacing, minNodeSpacing * 2.5); // Minimum reasonable spacing
  }

  private checkForOverlaps(): Array<{node1: string, node2: string, distance: number}> {
    // Check for node overlaps and return list of conflicting pairs
    const overlaps: Array<{node1: string, node2: string, distance: number}> = [];
    const nodeSize = this.options.nodeSize; // Approximate node size
    const minDistance = nodeSize + 10; // Minimum safe distance
    
    const allNodeIds = Array.from(this.nodeYPositions.keys());
    
    for (let i = 0; i < allNodeIds.length; i++) {
      for (let j = i + 1; j < allNodeIds.length; j++) {
        const node1 = allNodeIds[i];
        const node2 = allNodeIds[j];
        const y1 = this.nodeYPositions.get(node1)!;
        const y2 = this.nodeYPositions.get(node2)!;
    ...
    if (this.debug && overlaps.length > 0) {
      console.log(`⚠️  Found ${overlaps.length} potential overlaps:`);
      overlaps.forEach(overlap => {
        console.log(`   ${overlap.node1} ↔ ${overlap.node2}: ${overlap.distance.toFixed(1)}px (min: ${minDistance}px)`);
      });
    }
  ...
  private repositionSharedInputsOptimally(
    inEdges: Map<string, string[]>,
    outEdges: Map<string, string[]>,
    processedNodes: Set<string>
  ): void {
    // Find all leaf inputs and detect which ones are shared
    const leafInputs: string[] = [];
    this.processedNodes.forEach(node => {
      if (node.shape === 'rect') {
        // Check if this is a leaf input (no incoming edges from operations)
        const hasOperationInputs = (inEdges.get(node.id) || []).some(inputId => {
          const inputNode = this.processedNodes.find(n => n.id === inputId);
          return inputNode?.shape === 'circle';
        });
    ...
    // Detect shared inputs again (since we need fresh analysis)
    const sharedInputToTargets = this.detectSharedInputs(leafInputs, inEdges, outEdges);
    
    if (sharedInputToTargets.size === 0) {
      if (this.debug) console.log(`🔗 No shared inputs found for repositioning`);
      return;
    }
    
    if (this.debug) console.log(`🔗 Second pass: repositioning ${sharedInputToTargets.size} shared inputs...`);
    
    // Now that target nodes are positioned, reposition shared inputs at their centers
    sharedInputToTargets.forEach((targetIds, inputId) => {
      // Get positioned targets
      const positionedTargets = targetIds.filter(targetId => this.nodeYPositions.has(targetId));
      
      if (positionedTargets.length > 1) {
        // Calculate center Y of the targets
        const targetYs = positionedTargets.map(targetId => this.nodeYPositions.get(targetId)!);
        const minY = Math.min(...targetYs);
        const maxY = Math.max(...targetYs);
        const centerY = (minY + maxY) / 2;
        
        // Update shared input position
        const oldY = this.nodeYPositions.get(inputId);
        this.nodeYPositions.set(inputId, centerY);
        
        if (this.debug) console.log(`🔗 Repositioned shared input ${inputId}: ${oldY?.toFixed(1)} → ${centerY.toFixed(1)} (center of [${positionedTargets.join(', ')}])`);
      } else {
        if (this.debug) console.log(`🔗 Skipping ${inputId}: only ${positionedTargets.length} positioned targets`);
      }
    });

    // Now handle shared targets (nodes that receive from multiple sources)
    if (this.debug) console.log(`🎯 Checking shared targets: ${this.sharedTargetToSources.size} targets stored`);
    if (this.sharedTargetToSources.size > 0) {
      if (this.debug) console.log(`🎯 Repositioning ${this.sharedTargetToSources.size} shared targets...`);
      
      this.sharedTargetToSources.forEach((sourceIds, targetId) => {
        // Get positioned sources
        const positionedSources = sourceIds.filter(sourceId => this.nodeYPositions.has(sourceId));
        if (this.debug) console.log(`🎯 Target ${targetId}: ${sourceIds.length} sources, ${positionedSources.length} positioned`);
        
        if (positionedSources.length > 1) {
          // Calculate center Y of the sources
          const sourceYs = positionedSources.map(sourceId => this.nodeYPositions.get(sourceId)!);
          const minY = Math.min(...sourceYs);
          const maxY = Math.max(...sourceYs);
          const centerY = (minY + maxY) / 2;
          
          // Update shared target position
          const oldY = this.nodeYPositions.get(targetId);
          this.nodeYPositions.set(targetId, centerY);
          
          if (this.debug) console.log(`🎯 Repositioned shared target ${targetId}: ${oldY?.toFixed(1)} → ${centerY.toFixed(1)} (center of [${positionedSources.join(', ')}])`);
        } else {
          if (this.debug) console.log(`🎯 Skipping ${targetId}: only ${positionedSources.length} positioned sources`);
        }
      });
    } else {
      if (this.debug) console.log(`🎯 No shared targets to reposition`);
    }
    
    // Check for overlaps after repositioning
    const overlaps = this.checkForOverlaps();
    if (overlaps.length > 0) {
      if (this.debug) console.log(`⚠️  After repositioning: found ${overlaps.length} overlaps - attempting to resolve...`);
      this.resolveOverlaps(overlaps);
    }
  }

  private resolveOverlaps(overlaps: Array<{node1: string, node2: string, distance: number}>): void {
    // Attempt to resolve overlaps by slightly adjusting node positions
    const minSafeDistance = this.options.nodeSize + 15; // Safe minimum distance
    
    overlaps.forEach(overlap => {
      const y1 = this.nodeYPositions.get(overlap.node1)!;
      const y2 = this.nodeYPositions.get(overlap.node2)!;
      
      if (Math.abs(y2 - y1) < minSafeDistance) {
        // Move the nodes apart
        const midpoint = (y1 + y2) / 2;
        const adjustment = minSafeDistance / 2;
        
        if (y1 < y2) {
          this.nodeYPositions.set(overlap.node1, midpoint - adjustment);
          this.nodeYPositions.set(overlap.node2, midpoint + adjustment);
        } else {
          this.nodeYPositions.set(overlap.node1, midpoint + adjustment);
          this.nodeYPositions.set(overlap.node2, midpoint - adjustment);
        }
        
        if (this.debug) console.log(`🔧 Resolved overlap: ${overlap.node1} ↔ ${overlap.node2} (separated by ${minSafeDistance}px)`);
      }
    });
  }
  ...
  private renderSegmentedNode(node: GraphNode, segments: LabelSegment[], width: number, height: number, textColor: string): void {
    ...
    const fontSize = 11;
    const segmentPadding = 4; // Consistent with calculateNodeWidth
    
    // Start from the left edge with outer padding 
    let currentX = nodeX - width / 2 + segmentPadding;
    ...
    for (let i = 0; i < numSegments; i++) {
      ...
      if (hasExplicitWeights) {
        // Ratio-based mode: use proportional weights of inner width
        const totalInnerWidth = width - (segmentPadding * 2);
        segmentWidth = totalInnerWidth * (segment.weight || (1 / numSegments));
      } else {
        // Text-hugging mode: just the text width (no padding added here)
        const textWidth = this.estimateTextWidth(segment.text, fontSize);
        segmentWidth = Math.max(textWidth, fontSize * 2);
      }
      ...
      // Add consistent padding and divider after each segment (except the last)
      if (i < numSegments - 1) {
        currentX += segmentPadding; // Padding before divider
        ...
        currentX += 2; // Divider width
        currentX += segmentPadding; // Padding after divider
      }
    }
  }
}
